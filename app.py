...
def aceptar_viaje(id):
    try:
        ...
        if activo:
            conn.close()
            return jsonify({"status": "error", "message": "Ya tienes un viaje en curso"}), 200
...
# 👇 RUTA COMPLETADA A PARTIR DEL CORTE 👇
...
def api_verificar_status_viaje(viaje_id):
    ...
    if not viaje:
        return jsonify({"estado": "cancelado", "mensaje": "El viaje ya no existe"})
